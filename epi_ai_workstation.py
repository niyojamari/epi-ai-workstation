"""
Epi-AI Workstation — Full Production Build v2.1
================================================
All fixes applied:
  [v2.0] PHI/PII gate, Cox PH test, bivariate purposeful selection,
          DALY validation, logistic binary check, dropna notification,
          Cox covariate fix, decision matrix wizard, raw data preservation,
          dynamic report, BytesIO seek, inline SVG, SMR, PAF
  [v2.1] Stata DHS duplicate value-label fix (convert_categoricals fallback)
"""
...
with st.sidebar:
    ...
    if st.session_state["dataset"] is not None:
        df_sb = st.session_state["dataset"]
        st.markdown(
            f"**Active dataset:** `{df_sb.shape[0]:,}` rows × `{df_sb.shape[1]}` cols"
        )
        if st.button("⚠️ Reset to Raw Data"):
            ...
            st.rerun()
    st.warning(
        "⚠️ For de-identified research data only. "
        "Do not upload data containing personal identifiers."
    )
    st.caption("v2.1 | Production Build")
...
if app_mode == "📥 1. Data Ingestion & Cleaning":
    ...
    addresses, or any other direct identifiers. By uploading a file you confirm
    compliance with applicable data protection regulations (GDPR, HIPAA, or
    local equivalent).
    </div>
    """, unsafe_allow_html=True)
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["csv", "dta"],
        help="CSV and Stata .dta (versions 12–18) are supported.",
    )
    ...
    if uploaded_file:
        try:
            if uploaded_file.name.endswith(".csv"):
                ...
                # Primary attempt — full categorical conversion
                try:
                    df_new = pd.read_stata(uploaded_file)
                except ValueError:
                    # Fallback for DHS / survey files with duplicate value labels
                    # (e.g. district names repeated across urban/rural strata in v022)
                    ...
                    st.info(
                        "ℹ️ Some value labels in this Stata file are not unique "
                        "(common in DHS and national survey files — e.g. district "
                        "names in variable v022). Categorical columns have been "
                        "imported as numeric codes. Your data is fully intact and "
                        "all analyses will work normally."
                    )

            # Passive PHI column scanner
            phi_patterns = [
                "name", "surname", "dob", "birth", "address",
                "phone", "email", "ssn", "nid", "passport",
                "postcode", "zip", "contact",
            ]
            flagged = [
            ...
            if flagged:
                st.warning(
                    f"⚠️ Possible identifier columns detected: "
                    f"**{', '.join(flagged)}**. "
            ...
            st.session_state["rows_dropped"] = 0
            st.success(
                f"✅ Dataset loaded: {df_new.shape[0]:,} observations, "
                f"{df_new.shape[1]} variables."
            )
    ...
    if st.session_state["dataset"] is not None:
        ...
        if missing_vars:
            ...
            <strong>Imputation guidance:</strong> Median imputation is valid only for
            numeric predictor variables plausibly Missing Completely At Random (MCAR).
            ...
            if to_impute and st.button("▶ Run Imputation"):
                ...
                st.session_state["imputed_vars"] = list(
                    set(st.session_state["imputed_vars"] + to_impute)
                )
            ...
            st.markdown(
                '<div class="success-box">✅ No missing values detected.</div>',
                unsafe_allow_html=True,
            )
...
# ══════════════════════════════════════════════════════════════
    ...
    DECISION_MATRIX = {
        ("Continuous",               "Independent observations"):               ("OLS Linear Regression",                  "Robust SE recommended. Check residual normality via Q-Q plot."),
        ("Continuous",               "Nested / Clustered (repeated measures)"): ("Linear Mixed-Effects Model",             "Include random intercepts per cluster."),
        ("Continuous",               "Complex survey weights"):                 ("Survey-Weighted OLS",                    "Apply Taylor linearisation for SE."),
        ("Continuous",               "Matched / Case-Control"):                 ("Paired t-test or Conditional OLS",       "Condition on matched set. Do not use standard OLS."),
        ("Binary (0/1)",             "Independent observations"):               ("Logistic Regression",                    "Report OR with 95% CI. Run Hosmer-Lemeshow GOF."),
        ("Binary (0/1)",             "Nested / Clustered (repeated measures)"): ("Mixed-Effects Logistic Regression",      "Random intercepts per cluster. GEE for population-average estimates."),
        ("Binary (0/1)",             "Complex survey weights"):                 ("Survey-Weighted Logistic Regression",    "Use svyglm with quasibinomial family."),
        ("Binary (0/1)",             "Matched / Case-Control"):                 ("Conditional Logistic Regression",        "Standard logistic is BIASED for matched designs."),
        ("Time-to-Event (Survival)", "Independent observations"):               ("Cox Proportional Hazards",               "Verify PH assumption via Schoenfeld residuals."),
        ("Time-to-Event (Survival)", "Nested / Clustered (repeated measures)"): ("Shared-Frailty Cox Model",              "Add gamma frailty term per cluster."),
        ("Time-to-Event (Survival)", "Complex survey weights"):                 ("Weighted Nelson-Aalen / Cox",            "Survey weights applied at the risk set level."),
        ("Time-to-Event (Survival)", "Matched / Case-Control"):                 ("Stratified Cox Model",                   "Stratify on matched set ID."),
        ("Count",                    "Independent observations"):               ("Poisson or Negative Binomial Regression","Check overdispersion. Include offset for person-time."),
        ("Count",                    "Nested / Clustered (repeated measures)"): ("Mixed-Effects Poisson / NegBin",         "Random intercept per cluster."),
        ("Count",                    "Complex survey weights"):                 ("Survey-Weighted Poisson",                "Use svyglm with Poisson family + robust SE."),
        ("Count",                    "Matched / Case-Control"):                 ("Conditional Poisson (Mantel-Haenszel)", "Condition on matched strata."),
        ...
        ("Ordinal",                  "Nested / Clustered (repeated measures)"): ("Mixed-Effects Ordinal Regression",      "Account for clustering."),
        ("Ordinal",                  "Complex survey weights"):                 ("Survey-Weighted Ordinal Regression",     "Limited software support — use svyolr in R."),
        ("Ordinal",                  "Matched / Case-Control"):                 ("Stratified Ordinal Regression",          "Condition on matched strata."),
    }

    key          = (outcome, structure)
    method, note = DECISION_MATRIX.get(
        key, ("Consult a biostatistician", "This combination requires bespoke modelling.")
    )

    small_warnings = []
    if "Small" in sample and "Logistic" in method:
        small_warnings.append(
            "Small sample + logistic: consider Firth's penalised likelihood "
            "to avoid separation bias."
        )
    if "Small" in sample and "Cox" in method:
        small_warnings.append(
            "Small sample + survival: fewer than 10 events per variable "
            "is an overfitting risk — reduce covariates."
        )
    if "Small" in sample and "Mixed" in method:
        small_warnings.append(
            "Small sample + mixed effects: estimates will be unstable. "
            "Consider a simpler fixed-effects approach."
        )
    ...
    with st.expander("Why this recommendation?"):
        ...
        Decision follows Hosmer & Lemeshow (2013) model-selection criteria
        and standard epidemiological practice.
        Always verify model assumptions after fitting.
        """)
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        model_type = st.radio(
            "Model family:", ["Linear (OLS)", "Logistic (Binary)"], horizontal=True
        )
        ...
        if st.button("▶ Fit Regression Model"):
            if not x_vars:
                ...
                if n_dropped > 0:
                    st.warning(
                        f"⚠️ Listwise deletion: {n_dropped:,} rows removed "
                        f"({n_dropped / len(df_full) * 100:.1f}% of dataset). "
                        f"Analysis on {len(df_model):,} complete cases."
                    )
                ...
                if "Logistic" in model_type:
                    ...
                    if not (
                        ...
                        st.error(
                            ...
                            f"'{y_var}' has {len(unique_vals)} unique values: "
                            f"{sorted(unique_vals)[:10]}."
                        )
                        st.stop()
                ...
                try:
                    ...
                    if "Linear" in model_type:
                        ...
                        with st.expander("📊 Residual Diagnostics"):
                            ...
                            axes[0].scatter(
                                model.fittedvalues, residuals, alpha=0.4, s=20
                            )
                        ...
                        st.session_state["analysis_log"].append({
                            ...
                            "detail": (
                                f"Y={y_var}, X={x_vars}, N={len(df_model):,}, "
                        ...
                        with st.expander("📊 Hosmer-Lemeshow Goodness-of-Fit"):
                            pred    = model.predict()
                            ...
                            hl_stat = (
                                ((obs_1 - exp_1) ** 2 / (exp_1 * (1 - exp_1 / n_grp)))
                                .dropna().sum()
                            )
                            hl_p = 1 - stats.chi2.cdf(hl_stat, df=8)
                            ca, cb = st.columns(2)
                            ca.metric("H-L χ² statistic", f"{hl_stat:.3f}")
                            cb.metric("P-value (df=8)",   f"{hl_p:.4f}")
                            if hl_p < 0.05:
                                st.warning(
                                    "⚠️ P < 0.05: model fit may be poor. "
                                    "Reconsider model specification."
                                )
                            else:
                                st.success(
                                    "✅ P ≥ 0.05: no significant lack of fit detected."
                                )
    ...
    with tab2:
        ...
        if st.button("▶ Run Cox PH Model"):
            ...
            if covars:
                epv = n_events / len(covars)
                if epv < 10:
                    st.warning(
                        f"⚠️ Events-per-variable (EPV) = {epv:.1f}. "
                        "EPV < 10 is an overfitting risk — reduce covariates."
                    )
            ...
            try:
                ...
                with st.expander("📊 Hazard Ratios with 95% CI"):
                    hr_df = pd.DataFrame({
                        "HR":           np.exp(cph.params_),
                        "95% CI Lower": np.exp(
                            cph.confidence_intervals_["95% lower-bound"]
                        ),
                        "95% CI Upper": np.exp(
                            cph.confidence_intervals_["95% upper-bound"]
                        ),
                ...
                with st.expander(
                    "⚠️ Proportional Hazards Assumption Test (Required)", expanded=True
                ):
                    st.markdown("""
                    The PH assumption: hazard ratios must be **constant over time**.
                    **P < 0.05 per variable = PH violated.**
                    """)
                    try:
                        ph_test = proportional_hazard_test(
                            cph, df_cox, time_transform="rank"
                        )
                        ...
                        if not violated.empty:
                            st.error(
                                ...
                                "Consider time-varying coefficients, stratification, "
                                "or an accelerated failure time (AFT) model."
                            )
                        else:
                ...
                st.session_state["analysis_log"].append({
                    "step":   "Cox PH Model",
                    "detail": (
                        f"Time={time_col}, Event={event_col}, "
                        f"Covariates={covars}, N={len(df_cox):,}, Events={n_events}"
                    ),
                })
    ...
    with tab3:
        ...
        1. Each predictor is tested individually — variables with **p ≤ 0.25** advance.
        2. A multivariable model retains variables at **p ≤ 0.05** or those changing
           any other coefficient by ≥ 20% (confounding criterion).
        """)

        ps_outcome    = st.selectbox("Binary outcome variable (Y):", cols, key="ps_y")
        ps_candidates = st.multiselect("Candidate predictors:", cols, key="ps_x")
        ...
        if st.button("▶ Run Purposeful Selection"):
            if not ps_candidates:
                ...
                for var in ps_candidates:
                    try:
                        ...
                        screen_results.append({
                            ...
                            "Pass":     "✅ Yes" if min_p <= ps_screen_p else "❌ No",
                        })
                    except Exception:
                        screen_results.append({
                            "Variable": var,
                ...
                passed = [r["Variable"] for r in screen_results if r["Pass"] == "✅ Yes"]
                if not passed:
                    st.warning(
                        "No variables passed screening. "
                        "Try raising p₁ or reconsider variable selection."
                    )
                    st.stop()

                st.success(
                    f"**{len(passed)} variable(s) passed:** {', '.join(passed)}"
                )
                ...
                try:
                    ...
                    or_multi = pd.DataFrame({
                        "Odds Ratio":   np.exp(m_multi.params).round(4),
                    ...
                    retained = [
                        ...
                        if any(
                            c in idx
                            for idx in m_multi.pvalues[
                                m_multi.pvalues <= ps_final_p
                            ].index
                        )
                    ]
                    st.markdown(
                        f"**Final retained variables (p ≤ {ps_final_p}):** "
                        + (", ".join(retained) if retained
                           else "None — review confounding criterion")
                    )

                    st.session_state["analysis_log"].append({
                        "step":   "Bivariate Purposeful Selection",
                        "detail": (
                            f"Outcome={ps_outcome}, Screened={len(ps_candidates)}, "
                            f"Passed={len(passed)}, Retained={len(retained)}"
                        ),
                    })
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab_daly:
        ...
        **DALY = YLL + YLD**
        ...
        with col1:
            ...
            L = st.number_input(
                "Standard life expectancy at age of death (L, years):",
        ...
        with col2:
            ...
            DW = st.number_input(
                "Disability weight (DW, 0–1):",
                min_value=0.0, max_value=1.0, value=0.15, step=0.01,
                help="WHO GBD: 0 = full health, 1 = death equivalent.",
            )
            D = st.number_input(
                "Average duration of disease (D, years):",
        ...
        if N < 0 or I < 0:
            input_errors.append("Counts must be non-negative.")
        ...
        if input_errors:
            ...
            ca.metric("YLL", f"{yll:,.2f}")
            cb.metric("YLD", f"{yld:,.2f}")
            cc.metric("Total DALYs", f"{daly:,.2f}")
    ...
    with tab_smr:
        st.markdown("**Standardised Mortality Ratio (SMR)** = Observed / Expected deaths")
        obs = st.number_input("Observed deaths:", min_value=0, value=45, step=1)
        exp = st.number_input(
            "Expected deaths (from reference population):",
        ...
        if st.button("▶ Calculate SMR"):
            ...
            ci_hi = (
                (obs + 1)
                * (1 - 1 / (9 * (obs + 1)) + 1.96 / np.sqrt(9 * (obs + 1))) ** 3
            ) / exp
            ca, cb, cc = st.columns(3)
            ca.metric("SMR",            f"{smr:.3f}")
            cb.metric("95% CI lower",   f"{ci_lo:.3f}")
            cc.metric("95% CI upper",   f"{ci_hi:.3f}")
            if smr > 1:
                st.warning(
                    f"SMR > 1: study population mortality is "
                    f"{((smr - 1) * 100):.1f}% higher than the reference population."
                )
            else:
                st.success(
                    f"SMR < 1: study population mortality is "
                    f"{((1 - smr) * 100):.1f}% lower than the reference population."
                )
            st.session_state["analysis_log"].append({
                "step":   "SMR Calculation",
                "detail": (
                    f"Observed={obs}, Expected={exp:.1f}, "
                    f"SMR={smr:.3f} (95% CI: {ci_lo:.3f}–{ci_hi:.3f})"
                ),
            })
    ...
    with tab_paf:
        ...
        **Population Attributable Fraction (PAF)**
        ...
        """)
        pe = st.number_input(
            "Exposure prevalence in population (Pe, 0–1):",
            min_value=0.0, max_value=1.0, value=0.30, step=0.01,
        )
        rr = st.number_input(
            "Relative Risk or Odds Ratio (RR):", min_value=0.01, value=2.5, step=0.1
        )
        if st.button("▶ Calculate PAF"):
            paf = (pe * (rr - 1)) / (1 + pe * (rr - 1))
            st.metric("PAF", f"{paf * 100:.2f}%")
            st.info(
                f"Eliminating this exposure could theoretically prevent "
                f"**{paf * 100:.1f}%** of cases "
                "(assuming causality and complete exposure removal)."
            )
            st.session_state["analysis_log"].append({
...
# ══════════════════════════════════════════════════════════════
    ...
    if log:
        ...
        st.info(
            ...
            "Complete modules 1–4 to auto-populate the report."
        )
    ...
    if st.button("▶ Generate Report (.docx)"):
        ...
        if df_work is not None:
            ...
            doc.add_paragraph(
                f"Dataset dimensions: {df_work.shape[0]:,} observations "
                f"× {df_work.shape[1]} variables."
            )
            doc.add_paragraph(
        ...
        doc.add_paragraph(
            "Standard epidemiological limitations apply, including potential residual "
            "confounding, information bias, and selection bias. Causal inference should "
            "not be drawn from observational data without appropriate design and "
            "sensitivity analyses. Median imputation assumes MCAR; this assumption was "
            "not formally tested."
        )

        doc.add_heading("5. Data Governance", level=1)
        doc.add_paragraph(
            "The investigator confirmed prior to analysis that the dataset was "
            "de-identified and that its use complies with applicable data protection "
            "regulations (GDPR, HIPAA, or local equivalent). No personally identifiable "
            "information was processed by this tool."
        )
